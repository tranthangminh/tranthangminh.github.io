...
function clearTableOrder(tableId) {
    ...
    if (window.TinhTienGitHub && typeof window.TinhTienGitHub.markOrderDeleted === "function") {
        window.TinhTienGitHub.markOrderDeleted(tableId);
    }
    ...
    const createProductTile = (product, index) => {
        ...
        tile.innerHTML = `
            <button class="product-add" type="button" data-action="add" data-index="${index}" aria-label="Cộng ${productKey}">
                ...
                ${count > 0 ? `<span class="product-count">x${count}</span>` : ""}
...
document.getElementById("clearAll").addEventListener("click", () => {
    ...
    const storageKey = getOrderStorageKey();
    if (storageKey) {
        if (state.activeTable && window.TinhTienGitHub && typeof window.TinhTienGitHub.markOrderDeleted === "function") {
            window.TinhTienGitHub.markOrderDeleted(state.activeTable);
        }
        localStorage.removeItem(storageKey);
    }
    const paidStorageKey = getPaidStorageKey();
