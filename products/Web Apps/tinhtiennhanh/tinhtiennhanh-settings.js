(function () {
    ...
    function normalizeState(data) {
        ...
        if (data.orders) {
            if (Array.isArray(data.orders)) {
                data.orders.forEach((val, idx) => {
                    if (val !== null && val !== undefined) {
                        state.orders[String(idx)] = val;
                    }
                });
        ...
        if (data.paid) {
            if (Array.isArray(data.paid)) {
                data.paid.forEach((val, idx) => {
                    if (val !== null && val !== undefined) {
                        state.paid[String(idx)] = val;
                    }
                });
            } else if (typeof data.paid === "object") {
                Object.entries(data.paid).forEach(([key, val]) => {
                    if (val !== null && val !== undefined) {
                        state.paid[key] = val;
                    }
                });
        ...
        if (data.history) {
            if (Array.isArray(data.history)) {
                state.history = data.history.filter(item => item !== null && item !== undefined);
            } else if (typeof data.history === "object") {
                state.history = Object.values(data.history).filter(item => item !== null && item !== undefined);
            }
        }

        // 4. Normalize calcMode
        if (data.calcMode) {
            state.calcMode = String(data.calcMode);
        }
    ...
    function mergeStates(local, server) {
        ...
        allOrderIds.forEach(id => {
            ...
            const localVal = local.orders[id];
            const serverVal = server.orders[id];
            const baseVal = base.orders[id];
        ...
        allPaidIds.forEach(id => {
            ...
            const localVal = local.paid[id];
            const serverVal = server.paid[id];
            const baseVal = base.paid[id];
    ...
    function mergePendingServerData() {
        ...
        const normLocal = normalizeState(localData);
        const normServer = normalizeState(localDataPendingUpdate);
        const mergedData = mergeStates(normLocal, normServer);
    ...
    function applyFirebaseData(data) {
        ...
        const normLocal = normalizeState(localData);
        const normServer = normalizeState(data);
        const mergedData = mergeStates(normLocal, normServer);
        
        const localChanged = JSON.stringify(normLocal) !== JSON.stringify(mergedData);
        const serverChanged = JSON.stringify(normServer) !== JSON.stringify(mergedData);
    ...
    function handleFirebaseUpdate(snapshot) {
        ...
        const data = normalizeState(rawData);
    ...
    async function pushToFirebase() {
        ...
        const normLocal = normalizeState(localData);
        try {
            await dbRef.set(normLocal);
            updateStatusUI("synced", "Đã lưu lên đám mây!");
            const nodePath = localStorage.getItem(configKeys.nodePath) || "shopData";
            localStorage.setItem(`fastFoodCalculatorLastSync:${nodePath}`, JSON.stringify(normLocal));
        } catch (e) {
